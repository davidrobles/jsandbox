package dr.games.tictactoe;

import dr.games.core.MatchControllerObserver;
import dr.games.core.MoveObserver;
import static dr.games.tictactoe.TicTacToe.Cell;
import javax.swing.*;
import java.awt.*;
...
public class TicTacToeView extends JPanel implements MoveObserver, MatchControllerObserver<TicTacToe>
{
    ...
    protected void paintComponent(Graphics g)
    {
        ...
        for (int row = 0; row < TicTacToe.SIZE; row++)
        {
            ...
            {
                if (tic.getCell(row, col) != Cell.EMPTY)
                {
                    if (tic.getCell(row, col) == Cell.CROSS)
                        g2D.setColor(Color.BLACK);
                    else if (tic.getCell(row, col) == Cell.NOUGHT)
