...
public class Breakthrough extends AbstractGame implements Game<Breakthrough>
{
    ...
    public void makeMove(int move)
    {
        ...
        if (move < 0 || move >= movesArray.size())
        ...
        if (current == 0)
            black |= (1L << moveTaken[1]);
        else
            white |= (1L << moveTaken[1]);
...
//        current = starter = rng.nextInt(2);
    ...
    private void calculateMoves()
    {
        ...
        for (int i = 0; i < cells; i++)
        {
            ...
            {
                // DOWN
                if (((curBit << cols) & (black | white)) == 0L)
                    moves.add(new int[] { i, i + cols });

                // DOWN LEFT
//                if ((((curBit << (cols - 1)) & RIGHT_MASK) & ((black | white))) == 0L)
